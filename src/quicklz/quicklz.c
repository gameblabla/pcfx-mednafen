...
{
	// 1-8 bytes for aligning (not 0-7!); 8 bytes for buffersize (padds on 32 bit cpu); QLZ_HASH_SIZE hash table; QLZ_STREAMING_MODE_ROUNDED bytes streambuffer; optional QLZ_HASH_ENTRIES byte hash counter
	...
	unsigned char *streambuffer = buffer_aligned + 8 + QLZ_HASH_SIZE;
	unsigned int r;
	unsigned int compressed, base;
	unsigned char *hash_counter = streambuffer + QLZ_STREAMING_MODE_ROUNDED;

	if(size == 0 || size > 0xffffffff)
		return 0;

#if (QLZ_COMPRESSION_LEVEL == 0 && QLZ_STREAMING_MODE_ROUNDED == 0)
		memset((void *)hashtable, 0, QLZ_HASH_SIZE);
#endif
...
// if not QLZ_STREAMING_MODE, then QLZ_STREAMING_MODE_ROUNDED == 0 and first case (streaming buffer full) is executed unconditionally, functioning as block comp.
	if (*buffersize + size - 1 >= QLZ_STREAMING_MODE_ROUNDED)
	{
#if (QLZ_COMPRESSION_LEVEL == 0 && QLZ_STREAMING_MODE_ROUNDED != 0)
		memset((void *)hashtable, 0, QLZ_HASH_SIZE);
#endif
