...
int32 VDC::Run(int32 clocks, uint16 *pixels, bool skip)
{
 while(clocks > 0)
 {
  ...
  if(pixel_copy_count > 0)
  {
   if(!skip)
   {
	   /* Is it actually faster to use memcpy in this case rather than a loop ??? - Gameblabla*/
...
void VDC::DrawSprites(uint16 *target, int enabled)
{
	...
	for(int32_t i = (active_sprites - 1) ; i >= 0; i--)
	{
...
{
	if(sat_dma_counter <= 0 && !DMARunning && !mystery_phase)
	{
		if(pending_write)
		{
